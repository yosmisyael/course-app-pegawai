<main class="flex-1 overflow-y-auto bg-surface-low flex flex-col gap-4">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="p-4 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                @if(is_null($position->required_talents))
                                    <span class="px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs font-medium border border-blue-200">
                                ...
                                @else
                                    ...
                                    @endphp
                                    <span class="px-2 py-1 rounded text-xs font-medium border {{ $bgColor }} {{ $ratioColor }}">
                        ...
                        <td class="p-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center space-x-3">
                                <button wire:click="editPosition({{ $position->id }})" class="text-gray-400 hover:text-secondary">
                                    <span class="material-icons">edit_square</span>
                                </button>
                                <button wire:click="toggleDeleteModal({{ $position->id }})" class="text-gray-400 hover:text-red-500">
    ...
    {{--  Add/Edit Form  --}}
    <section class="h-screen w-1/3 {{ $isFormOpen ? 'translate-x-0' : 'translate-x-[100%]' }} transition-all duration-300 ease-out fixed right-0 top-0 z-10 bg-surface-high rounded-lg shadow-lg">
        <div class="p-6">
            <form wire:submit.prevent="savePosition" class="flex flex-col gap-4">
                ...
                <div class="input-group">
                    ...
                    <div class="relative mt-1">
                        ...
                        <input wire:model="required_talents" type="number" min="1" class="input-field" placeholder="Leave empty for Indefinite">
                    </div>
                ...
                <div class="input-group flex items-center justify-between border p-3 rounded bg-gray-50">
                    <label class="text-sm font-medium text-gray-700">Available for Assignment?</label>
                    <button type="button" wire:click="$set('status', '{{ $status === 'available' ? 'unavailable' : 'available' }}')"
                        ...
                        <span class="translate-x-0 pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out {{ $status === 'available' ? 'translate-x-5' : 'translate-x-0' }}"></span>
                    </button>
                </div>

                <div class="flex gap-3 justify-end mt-5">
                    <button wire:click="toggleForm" type="button" class="button-secondary w-fit">Cancel</button>
                    <button type="submit" class="button-primary w-fit">{{ $positionToEditId ? 'Save Changes' : 'Create Position' }}</button>
                </div>
            </form>
        </div>
    </section>
    ...
    @if($isDeleteModalOpen)
        <section class="fixed w-full h-screen left-0 top-0 bg-black/10 flex justify-center items-center z-50">
            <div class="bg-surface-high p-6 w-96 rounded-lg shadow-xl">
                ...
                <p class="text-gray-600 text-sm mb-6">Are you sure? This will remove the position definition. Active employees may be detached.</p>
                <div class="flex gap-3 justify-end">
                    <button wire:click="toggleDeleteModal" class="button-secondary">Cancel</button>
                    <button wire:click="deletePosition" class="button-danger border-gray-200 border-2 shadow-md">Delete</button>
                </div>
            </div>
        </section>
    @endif
